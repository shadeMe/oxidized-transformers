...
/// Mistral decoder (Jiang et al., 2023).
///
/// See:
/// * [Mistral 7B](https://arxiv.org/abs/2310.06825)
pub struct MistralDecoder;

/// HF Mistral model types
...
/// Hf Mistral decoder configuration.
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct HFMistralDecoderConfig {
...
impl TryFrom<HFMistralDecoderConfig> for TransformerDecoderConfig {
    type Error = BoxedError;

    fn try_from(hf_config: HFMistralDecoderConfig) -> Result<Self, Self::Error> {
        ...
        let mut feedforward = PointwiseFeedForwardConfig::default()
        ...
        if let Some(drop_p) = hf_config.hidden_dropout_prob {
            feedforward = feedforward.dropout(Box::new(DropoutConfig::default().p(drop_p)));
        }

        let mut attention = SelfAttentionConfig::default()
            .attention_scorer(Box::new(
                SDPAConfig::default().causal_mask_sliding_window(hf_config.sliding_window.clone()),
            ))
        ...
        if let Some(drop_p) = hf_config.attention_probs_dropout_prob {
            attention = attention.dropout(Box::new(DropoutConfig::default().p(drop_p)));
        }
...
impl FromHF for MistralDecoder {
    type Config = TransformerDecoderConfig;

    type HFConfig = HFMistralDecoderConfig;
...
mod tests {
    ...
    use crate::models::mistral::MistralDecoder;
    use crate::models::util::tests::{check_decoder, check_decoder_with_cache};

    #[test]
    #[report]
    fn mistral_decoder_gives_correct_output() -> Result<(), Whatever> {
        check_decoder::<MistralDecoder, _>(
        ...
        .whatever_context("Cannot check decoder with 'explosion-testing/mistral-test'")?;
        ...
        .whatever_context("Cannot check decoder 'explosion-testing/mistral-window-8'")
    }
}
